      ...
      if data[tostring(msg.to.id)] then
        ...
        elseif matches[1] == 'link' then
            if data[tostring(msg.to.id)]['link'] then
              ...
              return 'Invite link does not exist.\nTry !relink to generate.'
            end
          if matches[1] == 'relink' and is_mod(msg.from.id, msg.to.id) then
          ...
          if matches[2] == '+' then
            ...
            elseif matches[3] == 'm' and is_mod(msg.from.id, msg.to.id) then
                ...
                save_data(_config.moderation.data, data)
	             ...
	             elseif matches[3] == 's' and is_mod(msg.from.id, msg.to.id) then
	            if settings.sticker == 'yes' then
                return 'Anti Sticker Is Already enabled'
	            else
          ...
          elseif matches[2] == 'settings' and is_mod(msg.from.id, msg.to.id) then
            ...
            local text = 'Group settings:\n'
                  ...
                  ..'\n'..sticker_state..' Anti Sticker : '..settings.sticker
